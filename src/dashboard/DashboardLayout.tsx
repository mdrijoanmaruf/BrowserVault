...
export function DashboardLayout({ activePage, onNavigate, children, pageTitle, pageSubtitle }: DashboardLayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-[#0d0b1e] text-slate-900 dark:text-white overflow-hidden transition-colors duration-200" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <main className="flex-1 overflow-y-auto relative">
        ...
        <div className="absolute top-0 right-0 w-[400px] h-[160px] pointer-events-none hidden md:block overflow-hidden">
          ...
          <div className="absolute inset-0 opacity-20 dark:opacity-10" style={{ backgroundImage: 'radial-gradient(#5b32f5 1px, transparent 1px)', backgroundSize: '16px 16px', maskImage: 'radial-gradient(ellipse at top right, black 40%, transparent 70%)' }}></div>
        ...
        <div className="sticky top-0 z-50 bg-white/90 dark:bg-[#0d0b1e]/90 backdrop-blur-md border-b border-slate-200 dark:border-white/[0.06] px-10 py-8 transition-colors duration-200 flex justify-between items-end">
          <div>
            <h1 className="text-[28px] font-extrabold text-slate-900 dark:text-white leading-none tracking-tight">{pageTitle}</h1>
            {pageSubtitle && (
              <p className="text-[15px] text-slate-500 dark:text-white/50 mt-2 font-medium">{pageSubtitle}</p>
            )}
          </div>
          <button
            onClick={toggleTheme}
            ...
            title="Toggle Theme"
          >
            {settings.theme === 'dark' ? <LuSun className="w-5 h-5" /> : <LuMoon className="w-5 h-5" />}
          </button>
        </div>

        {/* Page content */}
        <div className="px-10 py-8 w-full relative z-20">
          {children}
        </div>
      </main>
    </div>
