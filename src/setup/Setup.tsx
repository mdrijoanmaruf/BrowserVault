...
export function Setup() {
  ...
  const handleStep2 = async () => {
    ...
    if (!email || !email.includes('@')) {
      setSetupError('Valid recovery email is required.');
      return;
    }
    setSaving(true);
    try {
      ...
      const res = await new Promise<any>((resolve) => {
        chrome.runtime.sendMessage(
          { action: 'REQUEST_OTP', payload: { email } },
          resolve
        );
      });
      if (res?.success) {
  ...
  const handleStep3 = async () => {
    setSetupError('');
    if (otp.length !== 6) {
      setSetupError('Enter the 6-digit OTP.');
      return;
    }
    setSaving(true);
    try {
      ...
      const res = await new Promise<any>((resolve) => {
        chrome.runtime.sendMessage(
          { action: 'VERIFY_OTP', payload: { otp, purpose: 'verify' } },
          resolve
        );
      });
      if (res?.success) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50 font-sans p-6">
        <div className="bg-white p-10 rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] max-w-md w-full text-center border border-slate-100">
          ...
          <h1 className="text-[22px] font-bold text-slate-900 mb-3 tracking-tight">
            Setup Complete!
          </h1>
          <p className="text-[13px] text-slate-500 mb-8 leading-relaxed">
            Your BrowserVault is now secure. You can close this tab and start
            using the extension securely.
          </p>
          <button
            onClick={() => window.close()}
            className="w-full bg-[#5a8bf7] hover:bg-[#4673d4] text-white text-[15px] font-semibold py-3.5 rounded-xl transition-all shadow-sm"
          >
            Close Setup
          </button>
        </div>
      ...
      <div className="bg-white p-8 sm:p-10 rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-slate-100 max-w-[440px] w-full">
        <div className="flex flex-col items-center mb-8">
          <img
            ...
            alt="Logo"
            className="w-16 h-16 rounded-2xl shadow-md mb-5"
          />
          <h1 className="text-[22px] font-bold text-slate-900 tracking-tight">
            Welcome to BrowserVault
          </h1>
          <p className="text-[13px] text-slate-500 mt-2 text-center leading-relaxed max-w-[280px]">
            Follow these quick steps to secure your browser and protect your
            privacy.
          </p>
        </div>
        ...
        <div className="flex justify-center gap-2 mb-8">
          {[1, 2, 3].map((step) => (
            <div
              key={step}
              className={`h-1.5 rounded-full transition-all duration-300 ${setupStep === step ? 'w-8 bg-[#5a8bf7]' : setupStep > step ? 'w-6 bg-slate-300' : 'w-2 bg-slate-200'}`}
            />
          ))}
        </div>
        ...
        {setupStep === 1 && (
          <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-[#5a8bf7] focus:ring-2 focus:ring-[#5a8bf7]/20 transition-all pr-12"
                  autoFocus
                />
                <button
                  type="button"
                  onClick={() => setShowPw((v) => !v)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-slate-400 hover:text-[#5a8bf7] transition-colors rounded-lg hover:bg-[#5a8bf7]/10"
                >
                  {showPw ? (
                    <svg
                      width="16"
            ...
            {setupError && (
              <p className="text-red-500 text-[12px] font-medium text-center bg-red-50 py-2 rounded-lg">
                {setupError}
              </p>
            )}
        ...
        {setupStep === 2 && (
          <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-right-8 duration-500">
            <div>
              ...
              <p className="text-[12px] text-slate-500 mb-3 ml-1 leading-relaxed">
                Required if you ever forget your master password.
              </p>
              <input
                type="email"
                value={email}
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-[#5a8bf7] focus:ring-2 focus:ring-[#5a8bf7]/20 transition-all"
                autoFocus
              />
            </div>
